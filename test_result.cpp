...
struct ThrowOnMove {
    ...
    ThrowOnMove& operator=(ThrowOnMove&&) noexcept { value = 0; return *this; }
};
...
TEST(is_ok_and_is_err_and) {
    ...
    // Test is_ok_and
    ASSERT(ok_result.is_ok_and([](int x) { return x == 42; }));
    ASSERT(!ok_result.is_ok_and([](int x) { return x == 0; }));
    ASSERT(!err_result.is_ok_and([](int) { return true; }));

    // Test is_err_and
    ASSERT(!ok_result.is_err_and([](const std::string&) { return true; }));
    ASSERT(err_result.is_err_and([](const std::string& s) { return s == "error"; }));
    ASSERT(!err_result.is_err_and([](const std::string& s) { return s == "other"; }));
}
...
TEST(and_then_operations) {
    ...
    // Test and_then on Ok
    auto result1 = ok_result.and_then([](int x) -> Result<int, std::string> {
    ...
    auto result2 = err_result.and_then([](int x) -> Result<int, std::string> {
        return make_ok<int, std::string>(x * 2);
    });
    ...
    auto result3 = ok_result.and_then([](int x) -> Result<std::string, std::string> {
...
TEST(or_else_operations) {
    ...
    // Test or_else on Ok
    auto result1 = ok_result.or_else([](const std::string&) -> Result<int, std::string> {
    ...
    // Test or_else on Err
    auto result2 = err_result.or_else([](const std::string&) -> Result<int, std::string> {
    ...
    auto result3 = err_result.or_else([](const std::string&) -> Result<int, std::string> {
...
TEST(map_operations) {
    ...
    // Test map on Ok
    auto result1 = ok_result.map([](int x) { return x * 2; });
    ASSERT(result1.is_ok());
    ASSERT_EQ(result1.unwrap(), 84);

    // Test map on Err
    auto result2 = err_result.map([](int x) { return x * 2; });
    ASSERT(result2.is_err());
    ASSERT_EQ(result2.unwrap_err(), "error"s);

    // Test map with type change
    auto result3 = ok_result.map([](int x) { return std::to_string(x); });
    ASSERT(result3.is_ok());
    ASSERT_EQ(result3.unwrap(), "42"s);
...
TEST(map_err_operations) {
    ...
    // Test map_err on Ok
    auto result1 = ok_result.map_err([](const std::string& s) { return s + " modified"; });
    ASSERT(result1.is_ok());
    ASSERT_EQ(result1.unwrap(), 42);

    // Test map_err on Err
    auto result2 = err_result.map_err([](const std::string& s) { return s + " modified"; });
    ASSERT(result2.is_err());
    ASSERT_EQ(result2.unwrap_err(), "error modified"s);

    // Test map_err with type change
    auto result3 = err_result.map_err([](const std::string& s) { return static_cast<int>(s.length()); });
    ASSERT(result3.is_err());
...
TEST(void_monadic_operations) {
    ...
    auto result1 = void_ok.and_then([]() -> Result<int, std::string> {
    ...
    auto result2 = void_err.and_then([]() -> Result<int, std::string> {
        return make_ok<int, std::string>(42);
    });
    ASSERT(result2.is_err());
    ASSERT_EQ(result2.unwrap_err(), "error"s);
    ...
    auto result3 = void_ok.and_then([]() -> Result<void, std::string> {
        return make_ok<void, std::string>();
    });
    ASSERT(result3.is_ok());

    // Test void and_then that returns error
    auto result4 = void_ok.and_then([]() -> Result<int, std::string> {
    ...
    auto result5 = void_ok.or_else([](const std::string&) -> Result<void, std::string> {
        return make_err<void, std::string>("should not execute"s);
    });
    ASSERT(result5.is_ok());

    // Test void or_else on Err - should call the function
    auto result6 = void_err.or_else([](const std::string& err) -> Result<void, std::string> {
    ...
    auto result7 = void_err.or_else([](const std::string&) -> Result<void, std::string> {
...
TEST(void_observers) {
    ...
    function_called = false;
    ASSERT(!void_err.is_ok_and([&]() -> bool {
        function_called = true;
        return true;
    }));
    ...
    function_called = false;
    ASSERT(!void_ok.is_err_and([&](const std::string&) -> bool {
        function_called = true;
        return true;
    }));
    ...
    function_called = false;
    ASSERT(void_err.is_err_and([&](const std::string& err) -> bool {
        function_called = true;
...
TEST(void_transformers) {
    ...
    auto result5 = void_ok.map_err([](const std::string& s) { return static_cast<int>(s.length()); });
    ASSERT(result5.is_ok());
    ...
    auto result6 = void_err.map_err([](const std::string& s) { return static_cast<int>(s.length()); });
    ...
    auto result7 = void_err.map_err([](const std::string& s) { return s + " transformed"; });
    ASSERT(result7.is_err());
...
TEST(void_chaining_operations) {
    ...
    auto result = make_ok<void, std::string>()
        .and_then([&]() -> Result<void, std::string> {
            step1_called = true;
            return make_ok<void, std::string>();
        })
        .and_then([&]() -> Result<void, std::string> {
        ...
        .or_else([&](const std::string& err) -> Result<void, std::string> {
    ...
    auto result2 = make_ok<void, std::string>()
        .map([]() { return 42; })           // void -> int
        .map([](int x) { return x * 2; })   // int -> int
        .and_then([](int x) -> Result<void, std::string> {  // int -> void
            ASSERT_EQ(x, 84);
            return make_ok<void, std::string>();
        });
...
TEST(void_reference_qualifiers) {
    ...
    // Test const lvalue reference
    {
        ...
        auto result = cref.map([]() { return 42; });
        ASSERT(result.is_ok());
        ASSERT_EQ(result.unwrap(), 42);
    }
    ...
    {
        auto result = std::move(void_ok).map([]() { return 42; });
        ASSERT(result.is_ok());
        ASSERT_EQ(result.unwrap(), 42);
    }
    ...
    {
        auto& ref = void_err;
        auto result = ref.map_err([](const std::string& s) { return s.length(); });
...

    {
        ...
        auto result = cref.is_err_and([](const std::string& s) { return s == "test"; });
        ASSERT(result);
    }

    {
        auto void_err_move = make_err<void, std::string>("move_test"s);
        auto result = std::move(void_err_move).map_err([](std::string&& s) { return s + "_moved"; });
        ASSERT(result.is_err());
...
TEST(chaining_operations) {
    auto result = make_ok<int, std::string>(5)
        ...
        .and_then([](int x) -> Result<int, std::string> {
            if (x > 8) {  // Changed from 15 to 8 to trigger error
...
TEST(reference_qualifiers) {
    ...
    {
        auto& ref = ok_result;
        auto result = ref.map([](const std::string& s) { return s.length(); });
        ASSERT(result.is_ok());
    ...
    // Test const lvalue reference
    {
        ...
        auto result = cref.map([](const std::string& s) { return s.length(); });
        ASSERT(result.is_ok());
        ASSERT_EQ(result.unwrap(), 5u);
    }
    ...
    // Test rvalue reference
    {
        auto result = std::move(ok_result).map([](std::string&& s) { return s.length(); });
        ASSERT(result.is_ok());
        ASSERT_EQ(result.unwrap(), 5u);
    }
}
...
TEST(edge_cases) {
    ...
    auto chained = same_type_ok.and_then([](const std::string& s) -> Result<std::string, std::string> {
